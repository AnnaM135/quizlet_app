  ...
  return (
    <div className={_._}>
        ...
        <form className={_.__select}>
            {levels?.map((item)=> (
                <label className={_.__select_option} key={item.level} htmlFor={`${item.level}`}>
                    <input className={_.__select_inp} type="radio" name="levels" id={`${item.level}`} onChange={(e) => setSelectedLevel(e.target.id)}/>
